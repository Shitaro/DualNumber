...
TEST(T_double, op_add_eq) {
    ...
    dn += 3.4;
    EXPECT_EQ(4.6, dn.real());
    EXPECT_EQ(3.4, dn.dual());

    dual_num::DualNumber<double> other(5.6, 7.8);
    dn += other;
    EXPECT_EQ(10.2, dn.real());
    EXPECT_EQ(11.2, dn.dual());
}
...
TEST(T_double, op_sub_eq) {
    ...
    dn -= 3.4;
    EXPECT_EQ(-2.2, dn.real());
    EXPECT_EQ(3.4, dn.dual());

    dual_num::DualNumber<double> other(5.6, 7.8);
    dn -= other;
    EXPECT_EQ(-7.8, dn.real());
    EXPECT_EQ(-4.4, dn.dual());
}
...
TEST(T_double, op_mul_eq) {
    ...
    dn *= 5.6;
    EXPECT_EQ(6.72, dn.real());
    EXPECT_EQ(19.04, dn.dual());

    dual_num::DualNumber<double> other(5.6, 7.8);
    dn *= other;
    EXPECT_EQ(37.632, dn.real());
    EXPECT_EQ(159.04, dn.dual());
}
...
TEST(T_double, op_div_eq) {
    ...
    dn /= 0.5;
    EXPECT_EQ(2.4, dn.real());
    EXPECT_EQ(6.8, dn.dual());
}
